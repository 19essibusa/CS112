...
void ListTester::testReadFromFile() const {
	...
	assert(list1.myFirst->myNext->myNext->myNext->myItem == 5);
	assert(list1.myLast->myItem == 6);
	cout << " 1 " << flush;

	List<double> list2;
	list2.readFrom("listTest2.0");
...
void ListTester::testInsertBefore() {
	...
	assert( l0.mySize == 1);
	assert( l0.insertBefore(4, 3) );
	assert( l0.mySize == 2 );
	assert( l0.myFirst->myItem == 3 );
	assert( l0.myLast->myItem == 4 );
	assert( l0.insertBefore(3, 2) );
	assert( l0.myFirst->myItem == 2 );
	assert( l0.myFirst->myNext->myItem == 3 );
	assert( l0.myLast->myItem == 4 );
	assert( l0.mySize == 3 );

	//Inserting a number to another number that is not in the list
	assert( !l0.insertBefore(6, 7) );
	assert( l0.mySize == 3 );

	// Testing whether they at the correct positions
	assert( l0.insertBefore(2, 1) );
	assert( l0.myFirst->myItem == 1 );
	assert( l0.myFirst->myNext->myItem == 2 );
	assert( l0.myFirst->myNext->myNext->myItem == 3 );
	assert( l0.myLast->myItem == 4 );
...
void ListTester::testInsertAfter() {
	cout << "Testing insertAfter()..." << flush;
		...
		//Testing a non-empty case
		l0.append(1);


		assert( l0.mySize == 1);
		assert( l0.insertAfter(1, 2) );
		assert( l0.mySize == 2 );
		assert( l0.myLast->myItem == 2 );
		assert( l0.insertAfter(1, 3) );
		assert( l0.myFirst->myNext->myItem == 3 );
		assert( l0.myLast->myItem == 2 );
		assert( l0.mySize == 3 );

		//Testing numbers that are not in the list
		assert( !l0.insertAfter(6, 7) );
		assert( l0.mySize == 3 );
